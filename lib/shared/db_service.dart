...
class StoredChatMessage {
  ...
  Future<List<String>> getLinkedRoomIdsForContact(String contactId) async {
    final dbx = database;
    final rows = await (dbx.select(dbx.contactChannelRoomLinks)
          ..where((t) => t.contactId.equals(contactId)))
        .get();
    final ids = <String>[];
    for (final r in rows) {
...
Future<void> deleteConversationLocal(String roomId) async {
    final dbx = database;
    // unlink any contact-channel links for this room
    await (dbx.delete(dbx.contactChannelRoomLinks)
          ..where((t) => t.roomId.equals(roomId)))
        .go();

    // delete conversation meta (if exists)
    await (dbx.delete(dbx.conversations)..where((t) => t.id.equals(roomId))).go();

    // delete messages for this room (raw SQL table)
    await dbx.customStatement(
      'DELETE FROM messages WHERE conversation_id = ?',
      [roomId],
    );
  }

  Future<void> deleteContactLocal(String contactId) async {
    final dbx = database;

    // find linked rooms to wipe messages
    final links = await (dbx.select(dbx.contactChannelRoomLinks)
          ..where((t) => t.contactId.equals(contactId)))
        .get();
    ...
    for (final l in links) {
      ...
      if (rid != null && rid.isNotEmpty) {
        await deleteConversationLocal(rid);
      }
    }
    ...
    // delete channels
    await (dbx.delete(dbx.contactChannels)
          ..where((t) => t.contactId.equals(contactId)))
        .go();

    // delete contact
    await (dbx.delete(dbx.contacts)..where((t) => t.id.equals(contactId))).go();
  }
}
